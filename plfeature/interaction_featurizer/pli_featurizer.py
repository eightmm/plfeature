...
class PLInteractionFeaturizer:
    ...
    def detect_metal_coordination(self) -> List[Interaction]:
        ...
        # Note: metal_coord is not in PHARMACOPHORE_SMARTS by default
        # This method will work if custom patterns are added
        protein_metal = self._protein_pharmacophores.get('metal_coord', set())
        ligand_metal = self._ligand_pharmacophores.get('metal_coord', set())

        for p_idx in protein_metal:
            for l_idx in ligand_metal:
                if p_idx < self.num_protein_atoms and l_idx < self.num_ligand_atoms:
                    dist = self._distance_matrix[p_idx, l_idx]
                    if dist < cutoff:
                        interactions.append(Interaction(
    ...
    def get_interaction_graph(self) -> Dict[str, Any]:
