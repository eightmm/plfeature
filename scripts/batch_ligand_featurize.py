#!/usr/bin/env python
"""
Batch feature extraction for ligand molecule files.
...
from plfeature.molecule_featurizer import MoleculeFeaturizer
...
SUPPORTED_EXTENSIONS = ['.sdf', '.mol2', '.mol', '.pdb']


def load_molecule(file_path: Path) -> Optional[Chem.Mol]:
    """
    Load molecule from file based on extension.
...
def process_single_ligand(
    ...
    try:
        ...
        save_dict = {
            ...
            'edge_index': edge['edge_index'],               # [2, N_edges]
            'adjacency': adj,                               # [N_atoms, N_atoms]
...
def process_wrapper(args: Tuple) -> Tuple[str, bool, str]:
    ...
    return process_single_ligand(*args)


def main():
    parser = argparse.ArgumentParser(
        description='Batch feature extraction for ligand molecule files'
    )
    parser.add_argument(
